...
id="kdbidstudi" value="{{$stankom->fbidstudi->id ??
old('kdbidstudi')}}">
 <input type="text" class="form-control
@error('bidangstudi') is-invalid @enderror" placeholder="Bidang Studi"
id="bidangstudi" name="bidangstudi" value="{{$stankom->fbidstudi-
>bidangstudi ?? old('bidangstudi')}}" aria-label="Bidang Studi" ariadescribedby="cari" readonly>
...
Cari Data Bidang Studi</button>
 ...
 <a href="{{route('standkomp.index')}}" class="btn
btn-default">
 Batal
 </a>
 </div>
...
btn-xs" onclick="pilih('{{$bs->id}}', '{{$bs->bidangstudi}}')" data-bsdismiss="modal">
 Pilih
 </button>
 </td>
 </tr>
...
@push('js')
 ...
 //Fungsi pilih untuk memilih data bidang studi dan mengirimkandata Bidang Studi dari Modal ke form edit
